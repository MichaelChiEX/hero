...
foreach(@MON_DATA){
	...
	for($i=0; $i<2; $i++){
		if ($abils[$i] ne ""){
			if($i == 0){
				$esk.="$ability_map{$abils[$i]}";
			}else{
				$esk.="<br><font color=green>$ability_map{$abils[$i]}</font>";
			}
		}
	}
	if ($ehprate) {
	...
	$mtable.="
		<tr class='monster' data-map=$ELEVEL[$elv] style='COLOR: #555555' bgColor=white style='font-size=10pt'>
			<td>$ELEVEL[$elv]</td>
			<td>$ename</td>
			<td>$ehp</td>
			<td>$emp</td>
			...
			<td>$ELE[$eele]</td>
			<td>$etec_name[0]</td>
			<td>$etec_name[1]$emprate</td>
			<td>$etec_name[2]$ehprate</td>
			<td>$esk</td>
		</tr>";
}
...
function showd(mapname) {
	document.querySelectorAll('.monster').forEach(function(element) {
		if(element.dataset.map===mapname){
