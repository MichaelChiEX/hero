sub equip2 {
	...
	if($it_no eq "bug"){&verchklog("$it_name,非法使用");}
	if(!$it_name){&error("資料出現異常。");}
	...
	<table border="0" width="80%" align=center height="150" class=FC>
		<tbody>
			...
			<tr>
				<td colspan=2 height=100 bgcolor="#330000"><font color="$FCOLOR2">$mess</font></td>
			</tr>
			<tr>
				...
				<form action="./status.cgi" method="POST">
					...
					<input type=submit class=FC value=回到裝備/使用畫面>
				</form>
				$BACKTOWNBUTTON
				</td>
			</tr>
