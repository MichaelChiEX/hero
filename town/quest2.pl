...
sub get_quest_gift{
	if($quest_count[0]<=19 || $quest_count[0] % 10 ne 0){return;}

	open(IN,"./logfile/item/$mid.cgi");
	@ITEM = <IN>;
	close(IN);
	$idx=int(($quest_count[0]-20)/10);
	$idx=$idx % 22;
	...
	$item=$tmp_itm[$idx];
	($it_no,$it_ki,$it_name,$it_val,$it_dmg,$it_wei,$it_ele,$it_hit,$it_cl,$it_sta,$it_type,$it_flg)=split(/<>/,$item);
	push(@ITEM,"$item\n");
	&questlog("<font color=green>[任務]</font>$quest_town_name的村民感謝<font color=blue>$mname</font>幫忙解決多次任務,贈送了$it_name");
	&maplog("<font color=green>[任務]</font>$quest_town_name的村民感謝<font color=blue>$mname</font>幫忙解決多次任務,贈送了$it_name");
	open(OUT,">./logfile/item/$mid.cgi");
	print OUT @ITEM;
	close(OUT);
	$com.="<br>§因為多次的解決村民的問題，特別贈送了$it_name";
}
1;
